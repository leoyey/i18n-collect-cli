        ...
        p = new Promise((resolve, reject) => {
          ...
          rl.on('line', (line) => {
            ...
            while (str) {
              ...
              if (['.vue'].includes(extname) && langObj[str]) {
                console.log(`Replacing '${str}' with key '${langObj[str]}'`)
                let props = []
                if(isTemplate) {
...
function parseJson (lang, langObj, pkey) {
  for (let cs_key in lang) {
    const cs_value = lang[cs_key];
    if (typeof cs_value === 'object' && cs_value !== null) {
      parseJson(cs_value, langObj, pkey + cs_key + '.')
    } else if (typeof cs_value === 'string') {
      langObj[cs_value] = pkey + cs_key
    }
  }
}
