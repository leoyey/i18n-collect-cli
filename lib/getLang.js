...
function inputLangs (fileName) {
  ...
  const parseLang = (cs_Lang, lang, otherObjs) => {
    ...
    for (item in lang) {
      ...
      } else if (typeof arr === 'object' && arr !== null && Object.keys(arr).length !== 0) {
        let child_lang = cs_Lang[item] ? cs_Lang[item] : {}
        let t = parseLang(child_lang, arr, otherObjs)
        total += t
        if (t > 0) {
          cs_Lang[item] = child_lang
        }
      }
    }
    return total
